...
export function renderGame(
  ...
  for (const portal of mapDef.portals) {
    const isLocked = portal.requiresQuest && !state.quests?.some(q => q.id === portal.requiresQuest && q.status === 'completed');
    drawPortal(ctx, portal, gameTime, isLocked);
  }
...
function drawPortal(ctx: CanvasRenderingContext2D, portal: any, time: number, isLocked: boolean = false) {
  const { sx, sy } = isoToScreen(portal.position.x, portal.position.y);
  const pulse = 0.5 + 0.5 * Math.sin(time * 3);
  const isDragon = (portal as any).variant === 'dragon';
  const portalColor = isLocked ? COLORS.portalLocked : isDragon ? '#dc2626' : COLORS.portal;
  const portalGlowColor = isLocked ? 'rgba(107, 114, 128, 0.2)' : isDragon ? 'rgba(220, 38, 38, 0.3)' : 'rgba(139, 92, 246, 0.3)';
  const scale = isDragon ? 1.6 : 1; // Dragon portal is bigger

  // Large outer glow
  const grad = ctx.createRadialGradient(sx, sy, 0, sx, sy, (60 + pulse * 20) * scale);
  grad.addColorStop(0, isLocked ? `rgba(107, 114, 128, ${0.2 + pulse * 0.1})` : isDragon ? `rgba(220, 38, 38, ${0.35 + pulse * 0.15})` : `rgba(139, 92, 246, ${0.35 + pulse * 0.15})`);
  grad.addColorStop(0.4, isLocked ? `rgba(75, 85, 99, ${0.15 + pulse * 0.05})` : isDragon ? `rgba(185, 28, 28, ${0.2 + pulse * 0.1})` : `rgba(192, 38, 211, ${0.2 + pulse * 0.1})`);
  ...
  ctx.translate(sx, sy);
  ctx.rotate(time * 1.5);
  ctx.strokeStyle = isLocked ? `rgba(107, 114, 128, ${0.3 + pulse * 0.2})` : isDragon ? `rgba(248, 113, 113, ${0.5 + pulse * 0.3})` : `rgba(196, 181, 253, ${0.5 + pulse * 0.3})`;
  ...
  ctx.rotate(-time * 3);
  ctx.strokeStyle = isLocked ? `rgba(75, 85, 99, ${0.2 + pulse * 0.1})` : isDragon ? `rgba(254, 202, 202, ${0.4 + pulse * 0.2})` : `rgba(237, 233, 254, ${0.4 + pulse * 0.2})`;
  ...
  if (!isLocked) {
    ...
    beamGrad.addColorStop(0, isDragon ? 'rgba(220, 38, 38, 0)' : 'rgba(139, 92, 246, 0)');
    beamGrad.addColorStop(0.5, isDragon ? 'rgba(220, 38, 38, 0.4)' : 'rgba(139, 92, 246, 0.4)');
    beamGrad.addColorStop(1, isDragon ? 'rgba(220, 38, 38, 0)' : 'rgba(139, 92, 246, 0)');
  ...
  if (!isLocked) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.fillStyle = isDragon ? `rgba(248, 113, 113, ${0.4 + 0.2 * Math.sin(time * 4 + i)})` : `rgba(196, 181, 253, ${0.4 + 0.2 * Math.sin(time * 4 + i)})`;
  ...
  // Label
  ctx.fillStyle = isLocked ? '#9ca3af' : isDragon ? '#fca5a5' : '#c4b5fd';
...
function drawPlayer(
  ...
  if (isAttacking) {
    const ps = isoToScreen(player.position.x, player.position.y);
    spawnParticles(ps.sx + 15, ps.sy - 15, 4, '#f59e0b', 'spark', 1.5);
    triggerShake(2, 0.1);
  }
}
